...
if(isset($_POST["submit_business"])){
    ...
    && isset($_POST["birth_year"]) && isset($_POST["gender"]) && isset($_POST["melicode"]) && isset($_POST["shcode"]) 
    ...
    header("location:transfer_document.php");
    exit;
    }else{
...
<html>
    ...
    // echo $show_next_div;
    ?>
    <body oncontextmenu="return false" onkeydown="return false;" onmousedown="return false;"y>
    <div class="w-100 background2 " >
                ...
                <div class="container mt-5 px-0" id="container" >
                    <form method="post" action="" class="px-0 d-block">
                        <div class="w-100 pr-3 h-100 text-right" style="overflow-y: scroll;">
                            <h4 class="text-danger text-right mt-5">توجه: پر کردن تمامی موارد الزامیست</h4>
                            <h2 class="text-right">:نوع وسیله نقلیه</h2>
                                <div class="personal-detail d-flex" dir="rtl">
                                    <input class=" pointer col-1 ml-2 mr-1" type="radio" name="kind" required>
                                    <h6 class="text-right mt-1">موتور</h4>
                                        <input class=" pointer ml-2 col-1 mr-1" type="radio" name="kind" required>
                                        <h6 class="text-right mt-1">وانت</h4>
                                            <input class=" pointer ml-2 col-1 mr-1" type="radio" name="kind" required>
                                            <h6 class="text-right mt-1">کامیون</h4>
                                            <input class=" pointer ml-2 col-1 mr-1" type="radio" name="kind" required>
                                    <h6 class="text-right mt-1">تریلر</h4>
                                </div>
                                <br><br>
                            ...
                            <div class="personal-detail text-right" dir="rtl">
                                <h4 class="ml-2">محدوده فعالیت</h4>
                                <select name="main_supply_category_id" data-select2-id="7" tabindex="-1" aria-hidden="true" required="" class="text-right personal-detail-select">
                                    <option value="1" data-select2-id="1">درون شهری</option>
                                    <option value="2" data-select2-id="2">برون شهری</option>
                                </select>
                            </div><br><br>
